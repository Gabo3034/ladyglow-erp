        ...
        if file_facturacion and file_comisiones:
            if st.button("💾 Procesar y Guardar en Base de Datos", type="primary"):
                with st.spinner('Procesando datos...'):
                    ...
                    # --- FUNCIÓN AUXILIAR PARA ALINEAR COLUMNAS ---
                    def alinear_columnas(df_nuevo, nombre_tabla, eng):
                        insp = inspect(eng)
                        if insp.has_table(nombre_tabla):
                            # Si la tabla ya existe, obtenemos sus columnas
                            db_cols = [c['name'] for c in insp.get_columns(nombre_tabla)]
                            # Filtramos el DataFrame para subir solo las columnas que la BD conoce
                            cols_a_mantener = [c for c in df_nuevo.columns if c in db_cols]
                            return df_nuevo[cols_a_mantener]
                        return df_nuevo # Si no existe, sube todo para crearla

                    # Alinear los DataFrames antes de enviarlos
